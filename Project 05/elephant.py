...
#Extension in the main file at line 482
...
def calcSurvival(parameter,population):
    ...
    for i in range(len(population)):
        ...
        if elephantAge < parameter[IDXJuvenileAge]:
        ...
        if parameter[IDXJuvenileAge] <= elephantAge <= parameter[IDXMaxAge]:
    ...
    for e in population:
        ...
        if gender == 'f' and parameter[IDXJuvenileAge] < age < parameter[IDXMaxAge]:
            ...
            # else
                ...
                if ( monthsContraceptive == 0 and random.random() < ( 1.0 /  (3.1 * 12 - 22)  ) ):
...
def main(argv):
    ...
    print("In An Average Year There Were: " + str(avgCalves) + " Calves, " + str(avgJuveniles) + " Juveniles, " + str(avgAdultMales) + " Male Adults, " + str(avgAdultFemales) +
    " Female Adults, " + str(avgSeniors) + " Seniors. " + str(numCulled) + " Elephants were culled")

    #Extension, this block of code below prints each individual result of the simulation into data.csv file and plots it into a graph using matplotlib 
