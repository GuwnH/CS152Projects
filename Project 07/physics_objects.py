...
class Block:
    ...
    def draw(self):
        '''Draws the block into the screen'''
        for item in self.vis:
            item.setFill("green")
            item.draw(self.win)
            
    def undraw(self):
        '''Removes block from the screen'''
        for item in self.vis:
            item.undraw()
    ...
    def collision(self,ball):
        '''This method returns a boolean value depending on whether or not the ball is touching/colliding with the block'''

        #Gets ball radius and position
        ballRadius = ball.getRadius()
        ballPosition = ball.getPosition()

        #Gets block position
        blockPosition = self.getPosition()
        
        #If statement that checks if the ball is touching the block
        if ballPosition[0] + ballRadius >= blockPosition[0] and ballPosition[0] - ballRadius <= blockPosition[0] + self.dx and ballPosition[1] + ballRadius >= blockPosition[1] and ballPosition[1] - ballRadius <= blockPosition[1] + self.dy:
